                ...
                <div class="card">
                    ...
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">تعديل تكلفه</h4>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <form action="{{ route('dashboard.opportunity_costs.update', $cost->id) }}" method="POST">
                                ...
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        <label class="form-label" for="fname">البند</label>
                                        <input type="text" class="form-control" id="fname" name="item" value="{{ $cost->item }}">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label class="form-label" for="lname">السعر</label>
                                        <input type="text" class="form-control" id="lname" name="price" value="{{ $cost->price }}">
                                    </div>
                                    <div class="form-group col-sm-12">
                                        <label class="form-label">الفرصه:</label>
                                        <select name="opportunity_id" class="selectpicker form-control" data-style="py-0">
                                            <option>اختر الفرصه</option>
                                            @foreach($opportunities as $opportunity)
                                            <!-- if the opportunity is selected -->
                                            <option value="{{ $opportunity->id }}" {{ $cost->opportunity_id == $opportunity->id ? 'selected' : '' }}>{{ $opportunity->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>

                                    <button type="submit" class="btn btn-primary">تعديل التكلفه</button>
                            </form>
                        </div>
                    </div>
                </div>
